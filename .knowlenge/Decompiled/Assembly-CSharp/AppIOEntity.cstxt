...
public abstract class AppIOEntity : IOEntity
{
	...
	public async void PairWithApp(RPCMessage msg)
	{
		...
		playerPairingData.Add("entityName", GetDisplayName().translated);
		NotificationSendResult notificationSendResult = await CompanionServer.Util.SendPairNotification("entity", player, GetDisplayName().translated, "Tap to pair with this device.", playerPairingData);
