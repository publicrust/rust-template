...
public class GrowableGenes
{
	...
	private GrowableGenetics.GeneType PickFavourableGeneType()
	{
		List<GrowableGenetics.GeneWeighting> list = slotWeights.Where((GrowableGenetics.GeneWeighting w) => GrowableGene.IsPositive(w.GeneType)).ToList();
		List<GrowableGenetics.GeneWeighting> list2 = slotWeights.Where((GrowableGenetics.GeneWeighting w) => !GrowableGene.IsPositive(w.GeneType)).ToList();
		float num = list.Sum((GrowableGenetics.GeneWeighting w) => w.Weighting);
		float num2 = list2.Sum((GrowableGenetics.GeneWeighting w) => w.Weighting);
		float num3 = Mathx.RemapValClamped(Mathf.Clamp(PlanterBoxStatic.FavourableGeneChance, 0f, 1f), 0f, 1f, 1f, 0f);
		float maxInclusive = num + num2 * num3;
		float num4 = Random.Range(0f, maxInclusive);
		float num5 = 0f;
		foreach (GrowableGenetics.GeneWeighting item in list)
		{
			num5 += item.Weighting;
			if (num4 < num5)
			{
		...
		foreach (GrowableGenetics.GeneWeighting item2 in list2)
		{
