    ...
    public class MineralEconomy : RustPlugin
    {
        ...
        private sealed class Configuration
        {
            ...
            public static Configuration DefaultConfig()
            {
                return new Configuration
                {
                    ...
                    {
                        new() { MinAmount = 0, MaxAmount = 10, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 50 } },
                        new() { MinAmount = 11, MaxAmount = 20, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 40 } },
                        new() { MinAmount = 21, MaxAmount = 30, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 30 } },
                        new() { MinAmount = 31, MaxAmount = 50, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 20 } },
                        new() { MinAmount = 51, MaxAmount = 10000, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 10 } }
                    },
                    BuyPrices = new List<PriceRange>
                    {
                        new() { MinAmount = 0, MaxAmount = 10, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 100 } },
                        new() { MinAmount = 11, MaxAmount = 20, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 80 } },
                        new() { MinAmount = 21, MaxAmount = 30, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 60 } },
                        new() { MinAmount = 31, MaxAmount = 50, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 40 } },
                        new() { MinAmount = 51, MaxAmount = 10000, Price = new() { ShortName = "scrap", SkinId = 0, Amount = 20 } }
                    },
        ...
        private void CheckPlayersDistance()
        {
            foreach (KeyValuePair<ulong, Recycler> kvp in playerRecyclers.ToList())
            {
                ...
                float distance = Vector3.Distance(player.transform.position, recycler.transform.position);
                if (distance > MaxDistanceFromRecycler)
                {
                    DestroyUI(player);
                    SendReply(player, GetLocalizedMessage("TooFarFromRecycler", player.UserIDString));
                    _ = playerRecyclers.Remove(kvp.Key);
                }
            }
        }
            ...
            try
            {
                ...
                CuiElementContainer container = new()
                {
                    // Основная панель
                    {
                        new CuiPanel
                        {
                            Image = { Color = HexToRustFormat("#3c3a34"), Material = "assets/content/ui/uibackgroundblur-ingame.mat" },
                    ...
                    // Верхняя панель заголовка
                    {
                        new CuiPanel
                        {
                            Image = { Color = HexToRustFormat("#524e49"), Material = "assets/content/ui/uibackgroundblur-ingame.mat" },
                    ...
                    // Баланс минералов с дополнительной информацией
                    {
                        ...
                        {
                            Text = {
                        Text = $"Текущий баланс рынка: {storedData.CurrentMineralAmount} минералов",
                        FontSize = 16,
                        Font = "robotocondensed-bold.ttf",
                        Align = TextAnchor.MiddleCenter,
                        Color = HexToRustFormat("#dcd3cc")
                    ...
                    // Заголовок таблицы с подробным описанием
                    {
                        ...
                        {
                            Text = {
                        Text = "СИСТЕМА ДИНАМИЧЕСКОГО ЦЕНООБРАЗОВАНИЯ\n" +
                ...
                for (int i = 0; i < config.SellPrices.Count; i++)
                {
                    ...
                    {
                        Text = {
                            Text = $"{config.SellPrices[i].MinAmount}-{config.SellPrices[i].MaxAmount} шт.",
                            FontSize = 12,
                            Font = "robotocondensed-regular.ttf",
                            Align = TextAnchor.MiddleCenter,
                            Color = HexToRustFormat("#dcd3cc")
        ...
        private void DestroyUI(BasePlayer player)
        {
            ...
            {
                PrintError($"Invalid UI arguments while destroying for {player.displayName}: {ex.Message}");
            }
            catch (NullReferenceException ex)
            {
                PrintError($"UI element not found while destroying for {player.displayName}: {ex.Message}");
            }
            catch (InvalidOperationException ex)
            {
                PrintError($"Invalid UI operation while destroying for {player.displayName}: {ex.Message}");
            }
        }
        #endregion UI
        ...
        private void OnLootEntity(BasePlayer player, StorageContainer container)
        {
            ...
            if (recycler != null)
            {
                ...
                {
                    SendReply(player, GetLocalizedMessage("NoPermissionButton", player.UserIDString));
                    return;
                }
